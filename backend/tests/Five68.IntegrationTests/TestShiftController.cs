...
public class TestShiftController
{
	...
	public TestShiftController(Five68WebAppFactory factory)
	{
		...
		_factory.SeedUser(EmployeeEmail, UserRole.Employee);
	}
	...
	private Task AuthorizeAsAsync(string email)
	{
		return _client.AuthorizeAsAsync(_factory, email);
	}

	private Guid CreateEmployee(string email)
	{
		return _factory.CreateEmployee(email);
	}

	private Guid SeedShift(Guid employeeId, DateOnly date, TimeOnly start, TimeSpan duration, Guid createdBy)
	{
		return _factory.SeedShift(employeeId, date, start, duration, createdBy);
	}

	private Guid GetUserId(string email)
	{
		return _factory.GetUserId(email);
	}
	...
	public async Task GetById_ExistingId_Returns200WithShift()
	{
		...
		HttpResponseMessage response = await _client.GetAsync($"/shift/{shiftId}");
	...
	public async Task GetById_AfterUpdate_UpdatedAtIsBumped()
	{
		...
		ShiftDTO? original = await (await _client.GetAsync($"/shift/{shiftId}")).Content.ReadFromJsonAsync<ShiftDTO>();

		await AuthorizeAsAsync(ManagerEmail);
		HttpResponseMessage updateResponse = await _client.PutAsJsonAsync($"/shift/{shiftId}", new ShiftUpdate
		{
		...
		HttpResponseMessage getResponse = await _client.GetAsync($"/shift/{shiftId}");
	...
	{
		await AuthorizeAsAsync(AdminEmail);
		HttpResponseMessage response = await _client.GetAsync($"/shift/{Guid.NewGuid()}");
	...
	public async Task GetByDateRange_ShiftsWithinRange_ReturnsOnlyThoseOrderedByDate()
	{
		...
		HttpResponseMessage response = await _client.GetAsync($"/shift?startDate={day1:yyyy-MM-dd}&endDate={day3:yyyy-MM-dd}");
	...
	{
		await AuthorizeAsAsync(AdminEmail);
		HttpResponseMessage response = await _client.GetAsync("/shift?startDate=2099-01-01&endDate=2099-01-31");
	...
	{
		await AuthorizeAsAsync(AdminEmail);
		HttpResponseMessage response = await _client.GetAsync("/shift?startDate=2031-02-10&endDate=2031-02-01");
	...
	{
		_client.DefaultRequestHeaders.Authorization = null;
		HttpResponseMessage response = await _client.GetAsync("/shift?startDate=2031-02-01&endDate=2031-02-10");
	...
	public async Task Create_ManagerValidShift_Returns201AndPersists()
	{
		...
		HttpResponseMessage response = await _client.PostAsJsonAsync("/shift", new ShiftCreate
		{
	...
	public async Task Create_MalformedJson_Returns400()
	{
		...
		HttpResponseMessage response = await _client.PostAsync("/shift", content);
	...
	public async Task Update_ManagerValidTimes_Returns200AndPersists()
	{
		...
		HttpResponseMessage response = await _client.PutAsJsonAsync($"/shift/{shiftId}", new ShiftUpdate
		{
			StartTime = newStart,
	...
	{
		_client.DefaultRequestHeaders.Authorization = null;
		HttpResponseMessage response = await _client.PutAsJsonAsync($"/shift/{Guid.NewGuid()}", new ShiftUpdate
		{
			StartTime = new TimeOnly(9, 0),
			Duration = TimeSpan.FromHours(8),
		});
	...
	public async Task Delete_ManagerExistingId_Returns204AndRemoves()
	{
		...
		HttpResponseMessage response = await _client.DeleteAsync($"/shift/{shiftId}");
	...
	{
		_client.DefaultRequestHeaders.Authorization = null;
		HttpResponseMessage response = await _client.DeleteAsync($"/shift/{Guid.NewGuid()}");
