...
using (IServiceScope scope = serviceProvider.CreateScope())
{
	...
	SeedData(db, adminPassword);
}

Console.WriteLine("Seeding complete.");

void SeedData(Five68DbContext db, string adminPassword)
{
	if (db.Users.Any())
	{
	...
	TimeOnly fullStart = new(18, 0);
	TimeSpan fullDuration = TimeSpan.FromHours(6);
	TimeOnly partialStart = new(19, 0);
	...
	DateOnly summerVacationStart = new(seedYear, 8, 8); // subito dopo la prima settimana di agosto
	List<DateOnly> summerVacation = Enumerable.Range(0, 14).Select(summerVacationStart.AddDays).ToList();
...
DateOnly EasterSunday(int year)
{
	int a = year % 19;
	int b = year / 100;
	int c = year % 100;
